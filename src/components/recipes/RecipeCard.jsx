...
const RecipeCard = ({ recipe ,addtoFav,isFav}) => {
  const { _id, name, image, cookTimeMinutes, prepTimeMinutes, difficulty, rating, description } = recipe;
    ...
    <div className="card group shadow-lg hover:shadow-2xl">
      <div className="relative overflow-hidden">
        ...
        <button
          ...
          className={`absolute top-3 right-3 p-2 ${isFav ? 'text-white bg-red-600/80' : 'text-gray-400 bg-white/80'}  rounded-full  transition-colors`}
          onClick={() => addtoFav(name, _id)}
        >
      ...
      <div className="p-4">
        ...
        <p className="text-text-secondary text-gray-600 text-sm line-clamp-2">
          {description || 'Delicious recipe made with fresh ingredients. Perfect for any occasion.'}
        </p>
      </div>
    </div>
  );
